...
def coverage_estimation(source_column, coverage_threshold):
    ...
    generator = PatternGenerator(source_column, coverage_threshold, sampling_size=int(0.2*len(source_column)))
...
for _ in range(1):
    ...
    for dataset in datasets:
        ...
        for column in gt_columns:
            ...
            average_threshold = 0
            # Coverage eatimation
            coverage_threshold = coverage_estimation(filtered_list, coverage_threshold)
            # Coverage update, re-generate
            generator = PatternGenerator(filtered_list, coverage_threshold, sampling_size=int(0.2*len(filtered_list)))
            generator.pattern_coverage_statictics()
