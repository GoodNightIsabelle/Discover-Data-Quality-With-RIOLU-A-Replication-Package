...
class TemplateGenerator:
    ...
    def template_generation(self, symbols, column):
        ...
        for template, token_stats in self.template_information.items():
            ...
            for _, stats in token_stats.items():
                ...
                if len(coverages) > 1:
                    ...
                    if categories_coverage/sum(coverages) >= self.coverage_threshold:
                ...
                for pos, char_stats in stats['chars'].items():
                    ...
                    '''
                    # if len(filtered.keys()) < self.categorical_threshold and int(pos[4:]) < minimum_constraint:
                    if  len(filtered.keys()) <= max(self.categorical_threshold*len(column), 2) and int(pos[4:]) < minimum_constraint:
                        char_list = sorted(list(filtered.keys()))
                        # Dump the last?
                        if last_type != '':
                            token_char += '%s{%d}'%(last_type, type_count)
                        if len(char_list) == 1:
                            token_char += re.escape(char_list[0])
                        else:
                            token_char += '(' + '|'.join([re.escape(char) for char in char_list]) + ')'
                        last_type = ''
                    '''
                    ...
                    chars_coverage = sum([coverages[i] for i in range(len(coverages)-2) if cluster_labels[i]==label_select])
                    if chars_coverage/sum(coverages) >= self.coverage_threshold:
                        # Dump the last?
                        if last_type != '':
                            token_char += '%s{%d}'%(last_type, type_count)
            ...
            self.templates.append(composed_template)

    def template_coverage_statictics(self):
        ...
        # Get templates
        self.template_generation(symbols, train_data)
        for template in self.templates:
            # Create a new dict for the template
            if template not in self.template_coverage.keys():
                self.template_coverage[template] = {}
            ...
            self.template_coverage[template] = cov_whole